...
public class CNIT255GroupProjectv3 extends javax.swing.JFrame {
 ...
 */
    ...
    public User(String name, boolean ban, boolean PMA){
        username = name;
        bannedStatus = ban;
        privateMessageAccess = PMA;
    }
    ...
    public class Member extends User{
        ...
        public Member(String Name,boolean ban, boolean PMA, String Password){
            super(Name,ban,PMA);
            password = Password;
        }
    ...
    public class Guest extends User{
        ...
        public Guest(String Name,boolean ban, boolean PMA, String idNumber){
            super(Name, ban, PMA);
            IDNumber = idNumber;
        }
    ...
    public class Admin extends Member{
         ...
         public Admin(String Name,boolean ban, boolean PMA, String Password){
            super(Name, ban, PMA, Password);
        }
 ...
 */
    ...
    private void BGuestLoginActionPerformed(java.awt.event.ActionEvent evt) {
       ...
       boolean allowed = true;
        ...
        if(allowed == true){ //when 'allowed == true' then the Guest user is logged in, the message GUI is enabled, and the Guest Login is exited
            ...
            AllUsers.add(new User(Username,false,true));
            DGuestLogin.dispose();
        }
    ...
    private void ExistingLoginActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        for(int i=0;i<AllMembers.size();i++){ 
            if (Username.equals(AllMembers.get(i).getusername())){ //searches to see if the username exists
                 if(!Password.equals(AllMembers.get(i).getpassword())){//confirms the password
                     WrongLogin.showMessageDialog(this,"Wrong Username or Password");
                 }
                 else{
                     if(Username.equals(BannedUsers.get(i).getusername())){//Checks to see if user is banned
                         Banned.showMessageDialog(this,"You have been banned from chat");  
                     }
                     else{
                         Message.setEnabled(true);
                         Send.setEnabled(true);
                         AllUsers.add(new User(Username,AllMembers.get(i).getbannedStatus(),AllMembers.get(i).getprivateMessageAccess()));
                     }
                 }
            }
            else if (i == AllMembers.size()){ //displays WrongLogin dialoge
                WrongLogin.showMessageDialog(this,"Wrong Username or Password");
            }
        }
    ...
    private void MLogoffActionPerformed(java.awt.event.ActionEvent evt) {
        System.exit(0);
        //Will remove User from the chat, AllUsers, and exit the Chatroom gui
    }
    ...
    private void BCreateActionPerformed(java.awt.event.ActionEvent evt) {
       String Username = NewUsername.getText(),Password = NewPassword.getText();
        for(int i=0;i<AllMembers.size();i++){ //Checks to make sure that the username isn't taken
            if (Username.equals(AllMembers.get(i).getusername())){ //searches to see if the username exists
                 WrongInfo.showMessageDialog(this, "Username taken");
            }
            else if(Password != NewConfirmPassword.getText())
                WrongInfo.showMessageDialog(this, "Your Passwords don't match");
            else{
                AllMembers.add(new Member(Username,false,false,Password));
            }
        }
    }
//</editor-fold>
    ...
    public static void main(String[] args) { //main method. Should just run the GUI?
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                CNIT255GroupProjectv3 GUI = new CNIT255GroupProjectv3();
                GUI.setVisible(true);
